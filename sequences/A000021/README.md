...
`a(n)` counts the distinct positive integers up to `2ⁿ` expressible as `x²+12y²` for non-negative
...
explained once, for the family, rather than redrawn here with `12` in place of `16`.

## Approach

`solution.mjs` sieves every value `x²+12y² ≤ bound` into a flat byte array and counts first-time
marks — the identical method A000018 uses, with `16` replaced by `12`. No table of published terms
is consulted.

Status: **matches OEIS exactly** for `a(0)..a(18)`, the published `%S`/`%T`/`%U` line fetched live
2026-09-02 from `oeis.org/search?q=id:A000021&fmt=text` (37 terms, offset 0). Verified by
[`proof.mjs`](proof.mjs), run live: every value from 1 to the bound independently re-tested by a
per-candidate perfect-square check (`v−12y²` a perfect square for some `y`), sharing no code with
the sieve, agreeing on every value through `n=18`, and matching OEIS term for term.

Full requirements and acceptance criteria:
[spec.md](../../memory-bank/specs/tasks/A000021.md).
...
deduplication is the real work) is identical here with `12` in place of `16`.
...
```
node sequences/A000021/solution.mjs 18    # compute a(0..18) from the definition
node sequences/A000021/proof.mjs 18       # re-check that output independently, value by value
```
...
the same collision-merging argument, the same growth chart — and applies unchanged to `x²+12y²`.
