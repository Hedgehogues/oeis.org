// A000024 — number of positive integers <= 2^n of the form x^2 + 10*y^2 (x, y >= 0 integers).
//
...
//
// IMPLEMENTATION. Computes a(n) from the definition: mark every value x^2+10y^2 up to the bound,
// count the DISTINCT marked values. No table of known answers is consulted.
//
// Run:  node sequences/A000024/solution.mjs [maxN]        (default 18)

export function countForm10(n) {
  const bound = 2 ** n;
  const marked = new Uint8Array(bound + 1);
  let count = 0;
  for (let x = 0; x * x <= bound; x++) {
    const rest = bound - x * x;
    for (let y = 0; 10 * y * y <= rest; y++) {
      const v = x * x + 10 * y * y;
