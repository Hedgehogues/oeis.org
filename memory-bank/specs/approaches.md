...
- `device` — the only context. It has no separate aggregate-root record: the catalog-level
  ...
  `_terms.md` holds only the entities — the twenty-two `[device::*]` records themselves.
...
- Every record MUST carry Essence / Recognized by / General case / Source, and MUST NOT carry a
  ...
  heading yields as many lines as there are device records (22); there is no `Used in:` field.
  Status: done.
...
- Every record MUST carry its own `Source:` field, honestly stating "no established name found"
  ...
  `grep -c '^Source:'` in `_terms.md` equals the number of device records (22). Status: done (22 of
  22; 16 honestly cite no established name for the diagram itself, some still sourcing the
